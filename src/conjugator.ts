import {
  Conjugation,
  Conjugator,
} from "../external/korean_conjugation/conjugator.ts";
import { Db, Tense, WordConjugation } from "./types.ts";

function filterConjugations(
...
function getRandomConjugation(db: Db, user_filters: Tense[]): WordConjugation {
  ...
  return {
    ...filtered_conjugations[
      Math.floor(Math.random() * filtered_conjugations.length)
    ],
    word,
  };
}

export function getRandomConjugations(
  count: number,
  tenses: Tense[],
  dbs: Db[],
): WordConjugation[] {
