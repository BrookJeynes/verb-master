...
function Home({
  ...
  function start(e: React.FormEvent<HTMLFormElement>) {
    ...
    const dbs = Object.values(config.datasets)
      .filter((db) => db.enabled)
      .map((db) => db.dataset);
    const tenses = Object.keys(config.tenses).filter(
      (tense) => config.tenses[tense as Tense].enabled,
    ) as Tense[];
  ...
  return (
    <div className="flex w-full flex-col items-center dark:bg-offblack dark:text-whitehover">
      ...
      >
        {error && (
          <div className="mb-10 w-2/3 rounded-md bg-redhover px-4 py-2 text-base">
            <p className="text-center font-bold text-whitehover shadow-sm">
              Error: {error}
            </p>
          </div>
        )}
        ...
        <div className="mb-8 mt-2 text-center">
          ...
          <h2 className="text-xl text-darkgray dark:text-whitehover">
            Your companion for Korean verbs
          </h2>
        </div>
        ...
        <div className="mt-12 flex w-full flex-col justify-center gap-24 rounded-md bg-lightgray px-5 py-6 dark:bg-jet md:flex-row">
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center">
              <input
                ...
                defaultChecked={Object.values(config.tenses).every(
                  (t) => t.enabled,
                )}
                onChange={(e) => {
                  ...
                  setConfig((prevConfig) => ({
                    ...
                    tenses: Object.fromEntries(
                      Object.entries(prevConfig.tenses).map(
                        ([key, value]): [string, TenseOption] => [
                          key,
            ...
            {Object.keys(config.tenses).map((tense_key) => {
              ...
              return (
                <div key={tense_key} className="flex items-center">
                  ...
                  />
                  <label htmlFor={tense_key}>
                    {tense_key[0].toUpperCase() + tense_key.substring(1)} (
                    {tense.example})
                  </label>
                </div>
              );
          ...
          <div className="flex flex-col gap-1">
                    ...
                    <FaRectangleXmark
                      ...
                      onClick={() => {
                        setConfig((prevConfig) => {
                          ...
                          const { [dataset_key]: _, ...updatedDatasets } =
                            prevConfig.datasets;
                          return {
                            ...prevConfig,
            ...
            <div className="mt-4 flex flex-col gap-2">
              <label className="font-bold" htmlFor="kimchiCsvUpload">
                Upload{" "}
                <a
                  className="underline hover:text-blue"
                  target="_blank"
                  href="https://kimchi-reader.app/stats/words/all"
                >
                  KimchiReader exported
                </a>{" "}
                CSV:
              </label>
              <input
        ...
        <footer className="mt-5 flex gap-2">
          <button
            aria-label="Dark/Light mode switcher"
            type="button"
            onClick={switchTheme}
          >
            {theme === "light" ? (
              <FaMoon className="text-blue" />
            ) : (
              <FaSun className="text-yellow" />
            )}
          </button>
          <p className="text-sm">
            This project is completely open-source! You can checkout the code{" "}
            <a
              className="underline hover:text-blue"
              target="_blank"
              href="https://github.com/brookjeynes/verb-master"
            >
              on GitHub here
            </a>
            .
